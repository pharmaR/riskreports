...
custom_package_report <- function(
    ...
    primary_color = "#007BFF", ## should move to brand.yml
    secondary_color = "#2A424E", ## should move to brand.yml
    ...
) {
  ...
  dirs_to_create <- unique(dirname(file.path(render_dir, files_to_copy)))
  dirs_to_create <- dirs_to_create[dirs_to_create != render_dir]  # Exclude the main directory
